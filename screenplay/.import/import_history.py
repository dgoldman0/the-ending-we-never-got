#!/usr/bin/env python3
"""One-time, hash-verified import. Never force-push or replace concurrent work."""
...
def attach(stream, base):
    """Parse fast-export framing; never substitute inside a data payload."""
    ...
    while line := src.readline():
        ...
        if line.startswith(b'data '):
            ...
            data = src.read(size)
            if len(data) != size:
                raise ValueError('Truncated fast-export payload')
            out.write(data)
...
def main():
    ...
    for part in ready['parts']:
        ...
        encoded = (inputs/'parts'/name).read_text()
    ...
    if g('tag','--list','reconstruction/*'):
        raise RuntimeError('Reconstruction tags already exist; manual reconciliation required')
    ...
    mapping = {old:mark_map[mark] for old,mark in originals.items()}
    record = {'attached_to':EXPECTED_BASE,'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
              'commit_map':mapping,'note':'Local reconstructed commits were reparented to the existing GitHub initialization commit; original recovery IDs remain in reconstruction.json.'}
    ...
    run([sys.executable,str(source.parent/'tools/verify.py')],cwd=repo)
    provenance=json.loads((repo/'screenplay/provenance/reconstruction.json').read_text())
    for milestone in provenance['milestones']:
        result=subprocess.run(['git','show',milestone['tag']+':'+milestone['path']],cwd=repo,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True).stdout
    ...
    published=g('ls-remote','origin','refs/heads/main').split()[0]
    if published!=tip:
    ...
    print(json.dumps({'published':tip,'history_commits':len(g('rev-list','HEAD').splitlines()),
                      'milestone_tags':len(refs),'source_sha256':SOURCE_SHA,
                      'root':g('ls-tree','--name-only','HEAD')},indent=2))
