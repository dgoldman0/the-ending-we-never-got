...
**Current runtime: 0.5.0-dev, interface and infrastructure pass (23 September 2026), not yet approved by the user.** Illustrated scenes (S001–S005) keep the whole painting, graded into a permanent light register by `../tools/grade-light.py`: bright (white-hot spilling glare, halation, colour splitting at the edges), ordinary (sallow light, cold shade, uneven exposure) or night (sunk light, cold shade, glowing sources), with local detail preserved. Behind the text the painting falls softly out of focus and into its own shadow colour; only the speaker is shown, in an oval portrait with a soft golden halo, graded to the same light. The reading controls are hidden behind a small gilt sun at the lower right. Scenes without paintings read as framed book pages with illuminated initials and the speaker in the margin. Every interface image is drawn in code by `../tools/build-ui-assets.py`. This is the maker's pass, not visual approval; the [failure reviews](../reviews/2026-09-23-window-failure/README.md) still apply.
