...
| --- | --- | --- | --- |
| Scene painting | `art/scenes/<file>.png` | 16:9, at least 1920×1080; 2560×1440 preferred | nothing; the scene lights up |
| Softened twin | `art/softened/scenes/<same file>.png` | same composition, re-graded | nothing; used when Softened is on |
| Portrait | `art/portraits/<set>-speaking.png`, `<set>-listening.png` | square, at least 512×512, transparent background | run `python3 visual-novel/tools/portrait-faces.py` |
...
- **Follow the three light registers,** and make the Softened twin the same picture re-graded, never a different one. The light column below is only a default from the scene heading. Choose the register in each scene's brief.
...
- **Use ordinary light.** Portraits appear in both lighting modes. A Softened twin under `art/softened/portraits/` is optional.
...
1. **The dead phone is invisible.** In `art/opening/cg/detail-phone.png` (Intense), the phone is nearly black, so Look closer shows an empty dark frame. Re-grade it so the phone's edge and screen read, while keeping the scene dark. The Softened version reads well.
