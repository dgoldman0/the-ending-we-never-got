...
- **Latest checkpoint is rejected and unfinished.** The user also rejected the partial replacement in `art/presentation-v2/`: too many empty backgrounds, missing character performances, an unconvincing interface and inadequate playthrough review. Disabling inconsistent sprites created missing cast coverage; it did not solve consistency. Preserve this checkpoint as work needing substantial revision, not an accepted visual baseline. The user's immediate instruction is to commit the progress with that status and discuss requirements before resuming production.
