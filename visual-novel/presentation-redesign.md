...
| Reader controls | One light for the original timeline (no lighting setting since 24 September 2026; "softened" is reserved for Stage II's regular light), coordinated scene/cast/detail grades; larger text, stable controls, keyboard access and visible focus in complete layouts |
...
| 4. Complete interface | Inspect all screen types with short/long text, larger type, three registers at 1280×720 and a larger window; assess smaller-window legibility | Cut frame edges, clipped glyphs, hidden gestures, moving controls, wrong hit areas or an incoherent screen family require a composed UI correction |
